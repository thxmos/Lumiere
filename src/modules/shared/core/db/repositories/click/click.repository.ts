...
export class ClickRepository implements IClickRepository {
  ...
  async getAllByLinkId(linkId: string): Promise<ClickResponse[]> {
    try {
      ...
      throw new RepositoryError("Failed to fetch clicks by link id", error);
    }
  }

  async getAllByUserId(userId: string): Promise<Click[]> {
    try {
      const socialClicks = await prisma.click.findMany({
        where: { userId },
      });
      const linkClicks = await prisma.click.findMany({
        where: { link: { LinkGroup: { User: { some: { id: userId } } } } },
      });
